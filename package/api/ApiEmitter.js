 ...
 * @LastEditors: web.王晓冬
 * @LastEditTime: 2020-04-14 14:56:35
 * @Description: api配置文件
 */
...
const createApi = (config, serviceName) => {
  ...
  let generateApiFunction = function (url, method, useMock, config = {}) {
    ...
    let fn = function (params, ...urlParams) {
      ...
      let callAPI = Api[method](serviceUrl + (userUrlParam ? urls.reduce((previousValue,
        currentValue, currentIndex) => {
        return currentIndex === 0 ? currentValue : previousValue + urlParams.shift() +
          currentValue
  ...
  for (let key in config) {
    ...
    result[key] = generateApiFunction(url, methods.toLowerCase(), useMock, {
      ...option
    })
  }
